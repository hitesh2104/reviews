...
class Candidate extends CI_Controller {
    ...
    public function candidate_excel_report($project_id) {
		$this->data['sales_excel_report'] = $this->candidate->projectSalesReport($project_id);
		$sales_excel_report = $this->data['sales_excel_report'];
	
		foreach($sales_excel_report as $row){$all_candidate[] = $row['candidate_id'];}
		if(!empty($all_candidate))
		{
			...
			$user_creadit_query = $this->db->query("SELECT credits,test_result from user where id=$loggedinUser");
			$user_credit_result = $user_creadit_query->result();
			$user_credit = $user_credit_result[0]->credits;
			$test_result = $user_credit_result[0]->test_result;
			$count = 0;
			
			//echo strpos($test_result, '16');
			//print_r($test_result);die;
			//if(!strpos($test_result, '16')){ it return 5, as it is like: if(!5){} so not going into if block.
            
            foreach($all_candidate as $id) {
                $testTemp = $this->db->query("SELECT test_result from user where id=$id");
                $test_string_temp = $testTemp->result();
                $test_result_temp = $test_string_temp[0]->test_result;
                if(!strpos($test_result_temp, '20')) {
                    $count = $count + 1;
                }
            }
			...
			if(strpos($test_result, '20')){
				...
				foreach($all_candidate as $id) {
					$test = $this->db->query("SELECT test_result from user where id=$id");
									
					$test_string = $test->result();
					$test_result = $test_string[0]->test_result;
					$new_test_result = "";
					
					if (empty($test_result)) {
						$new_test_result = '3,20';
					} else {
						$new_test_result = $test_result.',3,20';
					}
                    
                    if (!strpos($test_result, '20')) {
                        $this->db->query("UPDATE user SET test_result = '$new_test_result' WHERE id=$id");
                    }
				}
			}

			$this->load->view('pdf/IPT/sales_excel', $this->data);	
		}
		else
		{
			$this->load->view('pdf/IPT/sales_excel', $this->data);
    ...
    public function getResult4($uid) {
        ...
        if($D1 == $D2){
            ...
            $d_total = ($D1 - $D2);
            $resultArr['RD'] = $this->getDIPGRaphValue($d_total);
        }
        ...
        if($I1 == $I2){
            ...
            $I_total = ($I1 - $I2);
            $resultArr['RI'] = $this->getDIPGRaphValue($I_total);
        }
        ...
        if($S1 == $S2){
            ...
            $S_total = ($S1 - $S2);
            $resultArr['RS'] = $this->getDIPGRaphValue($S_total);
        }
        ...
        if($C1 == $S2){
            ...
            $C_total = ($C1 - $C2);
            $resultArr['RC'] = $this->getDIPGRaphValue($C_total);
        }
