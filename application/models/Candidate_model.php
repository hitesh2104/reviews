...
//    }
   ...
   public function getCandidateTestList($candidateId) {
    ...
    public function terms($post)
    {
        // print_r($post); die;
        $userId = $this->session->userdata('logged_in')['user_id'];
        $usedfor = (isset($post['usedfor']) && $post['usedfor']!="") ? $post['usedfor'] : "";
        $ackno1 = (isset($post['ackno1']) && $post['ackno1']!="") ? $post['ackno1'] : "";
        $ackno2 = (isset($post['ackno2']) && $post['ackno2']!="") ? $post['ackno2'] : "";
        $ackno3 = (isset($post['ackno3']) && $post['ackno3']!="") ? $post['ackno3'] : "";
        $ackno4 = (isset($post['ackno4']) && $post['ackno4']!="") ? $post['ackno4'] : "";
        $acknow_name = (isset($post['acknow_name']) && $post['acknow_name']!="") ? $post['acknow_name'] : "";
        $acknow_passport = $post['acknow_passport'];
        $raw_query = "INSERT INTO user_agreement(user_id, role, agreement_1, agreement_2, agreement_3, agreement_4, name, id_passport_no) VALUES ($userId,'$usedfor','$ackno1','$ackno2','$ackno3','$ackno4','$acknow_name', '$acknow_passport')"; 
        // echo $raw_query; die;
        $this->db->query($raw_query);
        return true;
    }
...
// Result
    ...
    public function select_summary($uid)
    {
       ...
       public function projectSalesReport($project_id){
        $candidate_ids = array();
        $query = $this->db->query("SELECT GROUP_CONCAT(DISTINCT `fk_candidate_id`) as candidate_ids FROM `candidate_associated_test` WHERE `fk_project_id` = '$project_id' AND `fk_test_id` = '20'");
        $query_result = $query->row_array();
        if($query_result['candidate_ids']){
            $tQuery = $this->db->query("SELECT `LAP_S_A`, `LAP_S_B`, `LAP_S_C`, `LAP_S_D`, `Sales_Value_Test`, `Sales_Apptitute_Test`, `Sales_Attribute_Test_A`, `Sales_Attribute_Test_B`,`open_text`,`open_text_res`,`candidate_id`,`sales_dip`,`first_name`,`last_name` FROM `test_given_answer` LEFT JOIN `user` as u ON `u`.`id` = `test_given_answer`.`candidate_id` WHERE `candidate_id` IN (".$query_result['candidate_ids'].") AND `LAP_S_A` != '' AND `LAP_S_B` != '' AND `LAP_S_C` != '' AND `LAP_S_D` != '' AND `Sales_Value_Test` != '' AND `Sales_Apptitute_Test` != '' AND `Sales_Attribute_Test_A` != '' AND `Sales_Attribute_Test_B` != '' AND `open_text` != '' AND `open_text_res` != '' AND `sales_dip` != ''");
            $tQuery_result = $tQuery->result_array();
            if($tQuery_result){
                $candidate_ids = $tQuery_result;
            }else{
                $candidate_ids = array();
            }
        }
