...
class Test_model extends CI_Model {
    ...
    public function creadit_varification($pid)
    {
        ...
        $user_credit=$user_credit_result[0]->credits=="" ? 0 : $user_credit_result[0]->credits ;
        //$user_MA=$this->get_MasterAdmin($loggedinUser);

        // if($test_credit > $user_credit)
        // {
        //     $user_credit = $this->getMasterCredit($user_MA);
        // }
        
        if($test_credit <= $user_credit)
        {
            // $this->session->set_userdata('test_master_user', $user_MA);
            return false;
        }
        else
        {
    ...
    public function updateCreadit($id)
    {
        ...
        // if($this->session->userdata['test_master_user']==0)
        // {
            $this->db->query("UPDATE user SET credits=credits-$test_credit WHERE id=$loggedinUser");
        // }
        // else
        // {
        //     $midd=$this->session->userdata['test_master_user'];
        //     $this->db->query("UPDATE user SET credits=credits-$test_credit WHERE id=$midd");
        //     $this->session->unset_userdata('test_master_user');
        // }
    ...
    public function processTest2($post) {
        ...
        if ($post['type']=="final") {
            $candidateTest=array();
            $percenatge = number_format((($correct / 50) * 10), 0);
            $candidateTest = array(
                'test_result' => $percenatge,
                'completed_date' => getCurrentDatetime(),
                'status' => 'completed'
    ...
    public function processTest3($post) {
        ...
        $result = $lap->result();
        $correctAnsArray = explode(',', $result[0]->$fldname);
        //echo "<pre>";print_r($correctAnsArray);die;
    ...
    {
        $test_query = $this->db->query("SELECT at.*,t.test_name from candidate_associated_test as at, test as t where fk_candidate_id=$id AND t.id=at.fk_test_id order by at.fk_test_id");
        return $test_query->result();
    }
    ...
    public function getQuestionList($secName = 'A'){
        $query = $this->db->query("SELECT questions_id, item FROM questions WHERE is_deleted = '0' AND `section` LIKE '".$secName."'");
        return $query->result_array();
    }
