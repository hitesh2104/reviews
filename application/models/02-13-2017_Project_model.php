...
class Project_model extends CI_Model {
                ...
                if ($candidateId) {
                    ...
                    if ($testInfo) {
                        if(@isset($args['regi']) == 1)
                        {
                            ...
                            $emaliFilename = $this->config->item('base_url') . "mailer/candidate-registration.php";
    ...
    public function getProjectList() {
        ...
        $loggedinUser = $this->session->userdata('logged_in')['user_id'];
        $query = $this->db->query("
            SELECT p.id, p.project_name, p.start_date, p.end_date, p.notification, p.status, p.is_launched
            FROM project AS p
    ...
    public function getProject($projectId) {
        if (!empty($projectId)) {
            $condition = " status != 'deleted' AND id = " . $projectId;
            $query = $this->db->query("
            SELECT *
            FROM project
