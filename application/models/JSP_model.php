...
class JSP_model extends CI_Model {
	...
	// Delete JSP
	public function deleteJSP($jspId) {
		$loggedIn = $this->session->userdata('logged_in');
		$loggedinUser = $loggedIn['user_id'];
		$masterUserId = $loggedIn['master_user_id'];
		if (isClient()) {
	...
	public function checkJobTitle($post, $isClone, $tableName = 'job_success_profile') {
		$loggedIn = $this->session->userdata('logged_in');
		$loggedinUser = $loggedIn['user_id'];
		$post['jspId'] = ($isClone == 0) ? $post['jspId'] : 0;
		$where = '';
		$id = 'j.id';
		if ($tableName == 'job_success_profile') {
			$id = 'j.jsp_id';
			$where .= " AND j.jsp_id != " . $post['jspId'];
		}
		$query = $this->db->query("
            SELECT $id, j.job_title, j.job_family
            FROM $tableName AS j
            WHERE j.status != 'deleted' AND j.created_by = $loggedinUser AND j.job_family like '" . $post['job_family'] . "'
				AND j.job_title like '" . $post['job_title'] . "' " . $where);
		return $query->result();
	}
