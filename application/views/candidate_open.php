...
foreach ($test_details as $value):
    ...
    if ($value->status == "completed" && ($value->fk_test_id == 1 || $value->fk_test_id == 2)) {$activeExeRpt++;}
    ?>
                                                                        <div class="checkbox">
                                                                            <label>
                        ...
                        <div class="row">
                            <div class="col-md-2" <?php echo ($isJsp == 1) ? '' : 'style="display:none;"'; ?> >
                                ...
                                <div class="complete text-center">
                                    ...
                                    <label class="" style="color:#3c8dbc; cursor: pointer; font-weight: 500 !important;" <?php echo ($isJsp == 1) ? ((count($test_details)) == $completedTest || $activeExeRpt > 0) ? 'onclick="excSubmit()"' : '' : 'onclick="excSubmit()"'; ?> >
                                    EXS</label>
                                    <?php }?>
                                    <br>
                                </div>
                            ...
                            <?php foreach ($test_details as $value): ?>
                            <?php
if (in_array($value->fk_test_id, [15, 16])) {
    $value->test_result2 = base64_encode($value->test_result2);
}
?>
                    ...
                    <?php 
                    if ($value->status == "completed") {
                      if(!is_numeric(trim($value->test_result)) && json_decode($value->test_result)!= null ) {
                        if(is_object(json_decode($value->test_result))){
                            $decoded_result = json_decode($value->test_result);
                            if(isset($decoded_result->sst_score)  && $decoded_result->sst_score!= ""){
                                $value->test_result = $decoded_result->sst_score;        
                            }
                            
                        }
                      }
                      ?>
                    <a style="margin-top: 50px; margin-bottom: 50px;"
                     target="_new" 
                     href="<?=base_url()?>candidate/download_report?u_name=<?=$general_details[0]->full_name?>&amp;title=<?=$value->test_name?>&amp;file=SST1&amp;score=<?=$value->fk_test_id == 13 ? '' : $value->test_result?>&amp;test_id=<?=$value->fk_test_id?>&amp;user_id=<?=$value->fk_candidate_id?> &amp;created_date=<?=$value->completed_date?>&amp; result2=<?=$value->test_result2?>" class="print_pdf_old">
                    ...
                    <?php }?>
                        ...
                        <?php if ($value->status == "completed") {echo "</a>";}?>
                                    <div class="complete text-center">
                                        <?php if ($value->status == "completed") {
                                          if(!is_numeric($value->test_result) && json_decode($value->test_result)!= null) {
                                            $decoded_value = json_decode($value->test_result);
                                            if(isset($decoded_value->sst_score) && $decoded_value->sst_score!=""){
                                                $value->test_result = $decoded_value->sst_score;    
                                            }
                                            
                                          }
                                          ?>
                                        <a style="margin-top: 50px; margin-bottom: 50px;" target="_new" href="<?=base_url()?>candidate/download_report?u_name=<?=$general_details[0]->full_name?>&amp;title=<?=$value->test_name?>&amp;file=SST1&amp;score=<?=$value->fk_test_id == 13 ? '' : $value->test_result;?>&amp;test_id=<?=$value->fk_test_id?>&amp;user_id=<?=$value->fk_candidate_id?> &amp;created_date=<?=$value->completed_date?>&amp; result2=<?=$value->test_result2?>" class="print_pdf_old"><?=$value->test_short_name?></a>
