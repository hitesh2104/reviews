...
<div class="content-wrapper">
	...
	foreach ($sectionOneArr as $i => $val) {
		$sn++;
		$checked = array();
		$style = array();
		...
		?>
                ...
                <a class="control_next btn btn-primary" <?=count($remember_list) > 0 ? '' : 'disabled'?>  style="opacity: 1;">Next</a>
...
// for ($i=1; $i <= count($sectionTwoArr) ; $i++) {
	foreach ($sectionTwoArr as $i => $val) {
		$sn++;
		$checked = array();
		$style = array();
		if (isset($remember_list[$i - 1])) {
...
<script type="text/javascript">
    $(document).ready(function ($) {
        ...
        if(remember==0 || remember=="" || remember==288)
        {
